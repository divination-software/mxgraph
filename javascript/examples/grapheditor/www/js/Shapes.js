...
(function()
{
	...
	mxUtils.extend(ExitShape, mxActor);
	ExitShape.prototype.size = 0.2;
	ExitShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s =  w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(s, 0), new mxPoint(w, 0), new mxPoint(w, h),
		                   new mxPoint(s, h), new mxPoint(0, h / 2)], this.isRounded, arcSize, true);
		c.end();
	};

	mxCellRenderer.prototype.defaultShapes['exit'] = ExitShape;
	...
	mxUtils.extend(DecisionShape, mxActor);
	DecisionShape.prototype.size = 0.2;
	DecisionShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s =  w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(w / 2, 0), new mxPoint(w, h / 2), new mxPoint(w / 2, h),
			 new mxPoint(0, h / 2)], this.isRounded, arcSize, true);
		c.end();
	};
	...
	mxUtils.extend(ModifyShape, mxActor);
	ModifyShape.prototype.size = 0.2;
	ModifyShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s =  w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var t = h * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(0, t), new mxPoint(s, 0), new mxPoint(w - s, 0), new mxPoint(w, t)
			, new mxPoint(w, h - t), new mxPoint(w - s, h), new mxPoint(s, h), new mxPoint(0, h - t)], this.isRounded, arcSize, true);
		c.end();
	};

	mxCellRenderer.prototype.defaultShapes['modify'] = ModifyShape;
	...
	mxUtils.extend(RecordShape, mxActor);
	RecordShape.prototype.size = 0.2;
	RecordShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s =  w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var t = h * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(0, t * 1.5), new mxPoint(s * 1.5 , 0), new mxPoint(w, 0), new mxPoint(w, h), new mxPoint(0, h)], this.isRounded, arcSize, true);
		c.end();
	};

	mxCellRenderer.prototype.defaultShapes['record'] = RecordShape;
	...
	mxUtils.extend(BatchShape, mxActor);
	BatchShape.prototype.size = 0.2;
	BatchShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s =  w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var t = h * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(0, 0), new mxPoint(w, t), new mxPoint(w, h - t), new mxPoint(0, h)], this.isRounded, arcSize, true);
		c.end();
	};

	mxCellRenderer.prototype.defaultShapes['batch'] = BatchShape;

	// Separate shape
	function SeparateShape()
	{
		mxActor.call(this);
	};
	mxUtils.extend(SeparateShape, mxActor);
	SeparateShape.prototype.size = 0.2;
	SeparateShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s =  w * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var t = h * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(0, t), new mxPoint(w, 0), new mxPoint(w, h), new mxPoint(0, h - t)], this.isRounded, arcSize, true);
		c.end();
	};
